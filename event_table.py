...
def call_event_table():
        ...
        with h5py.File(file_name,'r+') as f1:
            if wf_name1 in f1.keys():
                ...
                if wf_name2 in list(f1.keys()):
                    ...
                    evidence_dict[wf_name1] = ln_evidence
                ...
                if wf_name4 in list(f1.keys()):
                    ln_evidence = float(f1[wf_name4]['meta_data']['sampler']['ln_evidence'][0])
                    evidence_dict[wf_name3] = ln_evidence
                if wf_name5 in list(f1.keys()):
                    ln_evidence = float(f1[wf_name5]['meta_data']['sampler']['ln_evidence'][0])
                    evidence_dict[wf_name4] = ln_evidence
    ...
    event_table = list(zip(event_label, s_path_list, trigger_time, detectors, durations, waveform_list))
    return event_table
